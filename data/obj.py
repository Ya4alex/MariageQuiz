...
from data.question import QuestionManager, Result, LastQuestionError
...
from data.table.models import TableState, ClientRole, GameState, TableResult, ScreenResults
...
class Game:
    def __init__(self):
        self.tables: dict[int, Table] = {i: Table(self, i) for i in range(1, 4)}
    ...
    async def next_question(self):
        ...
        self.question_manager.current_question.start_timer()
        # Notify all clients about the game start
        await asyncio.gather(*[table.next_question() for table in self.tables.values()])
        await self.notify_screens()
...
class AdminClient:
    ...
    async def change_leader(self, event_data: dict):
        ...
        if table is None:
            await self.connection.send_json(ErrorEvent(error="Table not found").model_dump(mode='json'))
            return
        try:
    ...
    async def next_question(self, event_data: dict):
        ...
        except LastQuestionError:
            await self.connection.send_json(ErrorEvent(error="This is the last question.").model_dump(mode='json'))
            return
        except Exception as e:
            await self.connection.send_json(ErrorEvent(error=str(e)).model_dump(mode='json'))
...
class ScreenClient:
    ...
    def calc_results(self) -> ScreenResults | None:
        if self.game.state != GameState.in_results:
            return None
        table_results = [table.get_result() for table in self.game.tables.values() if table.get_result()]
        return ScreenResults(
            ...
            category_winners={
                category: max(
                    ...
                    default=None
                ) for category in self.game.question_manager.categories.keys() if not category.startswith('_')
            }
        )
...
"""
Ошибка: 1 validation error for TableResult table_name Input should be a valid string [type=string_type, input_value=None,
 input_type=NoneType] For further information visit https://errors.pydantic.dev/2.11/v/string_type
"""
